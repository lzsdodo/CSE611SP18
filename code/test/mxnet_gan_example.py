...
def visualize(fake, real, epoch):
    ...
    ax2.imshow(rbuff)
    fig.savefig('./images/mxnet/mnist_%d.png' % epoch)
    plt.show()
...
def main():
    ...
    for epoch in range(2):
        ...
        for i, batch in enumerate(image_iter):
            ...
            print('batch {} discriminator.forward for mnist image Not Dead'.format(i))
            ...
            discriminator.backward()
            print('batch {} discriminator.backward for mnist image Not Dead'.format(i))
            for gradsr, gradsf in zip(discriminator._exec_group.grad_arrays, gradD):
                for gradr, gradf in zip(gradsr, gradsf):
                    gradr += gradf
            #Update gradient on the discriminator
            discriminator.update()
            print('batch {} discriminator.update Not Dead'.format(i))
            ...
            generator.update()
            print('batch {} generator.update Not Dead'.format(i))
