...
g1 = mx.sym.Deconvolution(rand, name='g1', kernel=(8,8), num_filter=256, no_bias=no_bias)
gbn1 = mx.sym.BatchNorm(g1, name='gbn1', fix_gamma=fix_gamma, eps=epsilon)
gact1 = mx.sym.Activation(gbn1, name='gact1', act_type='relu')

g2 = mx.sym.Deconvolution(gact1, name='g2', kernel=(4,4), stride=(2,2), pad=(1,1), num_filter=128, no_bias=no_bias)
gbn2 = mx.sym.BatchNorm(g2, name='gbn2', fix_gamma=fix_gamma, eps=epsilon)
gact2 = mx.sym.Activation(gbn2, name='gact2', act_type='relu')

g3 = mx.sym.Deconvolution(gact2, name='g3', kernel=(4,4), stride=(2,2), pad=(1,1), num_filter=64, no_bias=no_bias)
gbn3 = mx.sym.BatchNorm(g3, name='gbn3', fix_gamma=fix_gamma, eps=epsilon)
gact3 = mx.sym.Activation(gbn3, name='gact3', act_type='relu')

g4 = mx.sym.Deconvolution(gact3, name='g4', kernel=(4,4), stride=(2,2), pad=(1,1), num_filter=3, no_bias=no_bias)
...
d4 = mx.sym.Convolution(dact3, name='d4', kernel=(8,8), num_filter=1, no_bias=no_bias)
d4 = mx.sym.Flatten(d4)

label = mx.sym.Variable('label')
discriminatorSymbol = mx.sym.LogisticRegressionOutput(data=d4, label=label, name='dloss')
...
def visualize(fake, real, index):
    #100x3x64x64 to 100x64x64x3
    ...
    print('successfully tranpose fake and real images')
    ...
    ax2.imshow(rbuff)
    fig.savefig('./images/mxnet/mnist_%d.png' % index)
    plt.close()
...
def main():
    ...
    send_notification('gan-mxnet', 'Training Starts')
    ...
    for epoch in range(2):
        ...
        for i, batch in enumerate(image_iter):
            ...
            if (i+1) % 50 == 0:
                print('epoch:', epoch, 'iter:', i+1, 'We are going to visualize images')
                visualize(outG[0].asnumpy(), batch.data[0].asnumpy(), i+1)
                print('Successfully saved image. Training time has been', (datetime.now()-starttime).seconds, 'seconds.')
    ...
    send_notification('gan-mxnet', 'Congrats! Completed. Training time {}'.format((datetime.now()-starttime).seconds)) 
