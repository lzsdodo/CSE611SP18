...
# MNIST
#show_n_images = 20
#mnist_images = utils.get_batch(glob(os.path.join(data_dir, 'mnist/*.jpg'))[:show_n_images], 28, 28, 'L')
#pyplot.imshow(utils.images_square_grid(mnist_images, 'L'), cmap='gray')
...
def show_generator_output(sess, n_images, input_z, out_channel_dim, image_mode):
    ...
    images_grid = utils.images_square_grid(samples, image_mode)
...
def gan_mnist():
    ...
    mnist_dataset = utils.Dataset('mnist', glob(os.path.join(data_dir, 'mnist/*.jpg')))
...
def gan_face():
    ...
    celeba_dataset = utils.Dataset('celeba', glob(os.path.join(data_dir, 'celeba/*.jpg')))
    with tf.Graph().as_default():
